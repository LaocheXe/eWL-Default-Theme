<?php
/*
 * e107 website system
 *
 * Copyright (C) 2008-2013 e107 Inc (e107.org)
 * Released under the terms and conditions of the
 * GNU General Public License (http://www.gnu.org/licenses/gpl.txt)
 *
*/
 
if (!defined('e107_INIT')) { exit; }
 
global $sc_style;

$sc_style['CPAGEAUTHOR|default']['pre'] = '';
$sc_style['CPAGEAUTHOR|default']['post'] = ", ";

$sc_style['CPAGESUBTITLE|default']['pre'] = '<h2>';
$sc_style['CPAGESUBTITLE|default']['post'] = '</h2>';

$sc_style['CPAGEMESSAGE|default']['pre'] = '';
$sc_style['CPAGEMESSAGE|default']['post'] = '<div class="clear"><!-- --></div>';

$sc_style['CPAGENAV|default']['pre'] = '<div class="f-right pull-right col-md-3">';
$sc_style['CPAGENAV|default']['post'] = '</div>';

#### default template - BC ####
	// used only for parsing comment outside of the page tablerender-ed content
	// leave empty if you integrate page comments inside the main page template
	
	
	$PAGE_TEMPLATE['default']['page'] = '
		{PAGE}
		{PAGECOMMENTS}
	'; 
	
	// always used - it's inside the {PAGE} sc from 'page' template
	$PAGE_TEMPLATE['default']['start'] = '<div id="{CPAGESEF}" class="cpage_body cpage-body">{CHAPTER_BREADCRUMB}'; 
	
	// page body
	$PAGE_TEMPLATE['default']['body'] = '
		{CPAGEMESSAGE|default}
		
		{CPAGESUBTITLE|default}
		<div class="clear"><!-- --></div>
		
		{CPAGENAV|default}
		{CPAGEBODY|default}
		
		<div class="clear"><!-- --></div>
		{CPAGERATING|default}
    
     <a target="_blank" href="'.SITEURL.'e107_admin/cpage.php?action=edit&id={CPAGEID}" class="fa fa-edit"> </a>
	'; 
	
	// used only when password authorization is required
	$PAGE_TEMPLATE['default']['authorize'] = '
		<div class="cpage-restrict ">
			{message}
			{form_open}
			<div class="panel panel-default">
				<div class="panel-heading">{caption}</div>
					<div class="panel-body">
					    <div class="form-group">
				       		 <label class="col-sm-3 control-label">{label}</label>
					        <div class="col-sm-9">
					               {password} {submit} 
					        </div>
			     		</div>
					</div>
      			</div>
			{form_close}
		</div>
	';
	
	// used when access is denied (restriction by class)
	$PAGE_TEMPLATE['default']['restricted'] = '
		{text}
	';
	
	// used when page is not found
	$PAGE_TEMPLATE['default']['notfound'] = '
		{text}
	';
	
	// always used
	$PAGE_TEMPLATE['default']['end'] = '{CPAGERELATED: types=page,news}</div>'; 
	
	// options per template - disable table render
//	$PAGE_TEMPLATE['default']['noTableRender'] = false; //XXX Deprecated
	
	// define different tablerender mode here
	$PAGE_TEMPLATE['default']['tableRender'] = 'cpage';

	
#### No table render example template ####
	$PAGE_TEMPLATE['custom']['start'] 			= '<div id="{CPAGESEF}" class="cpage-body">'; 
	$PAGE_TEMPLATE['custom']['body'] 			= ''; 
	$PAGE_TEMPLATE['custom']['authorize'] 		= '
	';
	
	$PAGE_TEMPLATE['custom']['restricted'] 		= '
	';
	
	$PAGE_TEMPLATE['custom']['end'] 			= '</div>'; 
	$PAGE_TEMPLATE['custom']['tableRender'] 	= '';
	
	
$sc_style['CPAGEAUTHOR|portfolio']['pre'] = '';
$sc_style['CPAGEAUTHOR|portfolio']['post'] = ", ";

$sc_style['CPAGESUBTITLE|portfolio']['pre'] = '<h2>';
$sc_style['CPAGESUBTITLE|portfolio']['post'] = '</h2>';

$sc_style['CPAGEMESSAGE|portfolio']['pre'] = '';
$sc_style['CPAGEMESSAGE|portfolio']['post'] = '<div class="clear"><!-- --></div>';

$sc_style['CPAGENAV|portfolio']['pre'] = '<div class="f-right pull-right col-md-3">';
$sc_style['CPAGENAV|portfolio']['post'] = '</div>';

#### portfolio template - BC ####
	// used only for parsing comment outside of the page tablerender-ed content
	// leave empty if you integrate page comments inside the main page template
	
	
	$PAGE_TEMPLATE['portfolio']['page'] = '
		{PAGE}
		{PAGECOMMENTS}
	'; 
	
	// always used - it's inside the {PAGE} sc from 'page' template
	$PAGE_TEMPLATE['portfolio']['start'] = ''; 
	
	// page body
	$PAGE_TEMPLATE['portfolio']['body'] = ' <div>
		<div class="row"> {CPAGEMESSAGE|portfolio} </div>
		
		{CPAGESUBTITLE|portfolio}
		<div class="clear"><!-- --></div>
		
		{CPAGENAV|portfolio}
            <div class="row">
                <div class="col-md-6">
                    {SETIMAGE: w=0&h=0}
                    <a href="{CMENUIMAGE=url}" class="thumbnail margin-bottom-20" data-lightbox="w1" title="{CMENUTITLE}">
                        {CMENUIMAGE}
                    </a>
 
                </div>
                <div class="col-md-6">
                    <h3 class="section-title">{CPAGETITLE}</h3>
                    {CPAGEBODY|portfolio}          
										 
                </div>
            </div>
		      
		
		<div class="clear"><!-- --></div>
		{CPAGERATING|portfolio}
    
     <a target="_blank" href="'.SITEURL.'e107_admin/cpage.php?action=edit&id={CPAGEID}" class="fa fa-edit"> </a>
	'; 
	
	// used only when password authorization is required
	$PAGE_TEMPLATE['portfolio']['authorize'] = '
		<div class="cpage-restrict ">  
			{message}
			{form_open}
			<div class="panel panel-portfolio">
				<div class="panel-heading">{caption}</div>
					<div class="panel-body">
					    <div class="form-group">
				       		 <label class="col-sm-3 control-label">{label}</label>
					        <div class="col-sm-9">
					               {password} {submit} 
					        </div>
			     		</div>
					</div>
      			</div>
			{form_close}
		</div>
	';
	
	// used when access is denied (restriction by class)
	$PAGE_TEMPLATE['portfolio']['restricted'] = '
	<div class="row">	<div class="alert alert-info">{text} </div>
	';
	
	// used when page is not found
	$PAGE_TEMPLATE['portfolio']['notfound'] = '
		{text}
	';
	
	// always used
	$PAGE_TEMPLATE['portfolio']['end'] = '{CPAGERELATED: types=page,news}</div>'; 
	
	// options per template - disable table render
//	$PAGE_TEMPLATE['portfolio']['noTableRender'] = false; //XXX Deprecated
	
	// define different tablerender mode here
	$PAGE_TEMPLATE['portfolio']['tableRender'] = 'cpage';


	
	
	
	







	
	
	
?>